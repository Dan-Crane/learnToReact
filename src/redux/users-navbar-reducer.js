...
export const setToggleIsProgress = (isFetching, userId) => ({type: TOGGLE_IS_FOLLOWING_PROGRESS, isFetching, userId})
...
export const follow = (userId) => {
	return (dispatch) => {
		dispatch(setToggleIsProgress(true, userId))
		usersAPI.follow(userId)
			.then(data => {
				if (data.resultCode === 0) {
					dispatch(followSuccess(userId))
				}
				dispatch(setToggleIsProgress(false, userId))
			})
	}
}
